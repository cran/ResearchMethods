...
#    ui = "\u03bci's:"
    ...
    refresh2 <- function(index=NULL,...){
        ...
        if(length(out)==2){
            ...
            text(0.25,0.50,out[1],col=1,pos=4,cex=1.7)
            text(0.25,0.45,out[2],col=1,pos=4,cex=1.7)
            text(0.28,0.45,out[3],col=1,pos=4,cex=1.7)
            text(0.32,0.47,out[4],col=6,pos=4,cex=1.7)
            text(0.25,0.25,out[5],col=1,pos=4,cex=1.7)
...
#            points(x,n8,type='l',lty=2,col='blue')
    ...
    alphaSlider <- tkscale(sliderFrame,command=refresh2,from=0.005,to=0.500,label=a,orient = "horiz",resolution=0.005,state='active',troughcolor=ccc)
    betaSlider <- tkscale(sliderFrame,command=refresh2,from=0.005,to=0.500,label=b,orient="horiz",resolution=0.005,troughcolor=ccc)
